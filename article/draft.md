...
- OpenTelemetry Swiftの公式Persistence Exporterは、iOSアプリの一時的な送信先
  障害に対する回収率を**0%→100%**に変えられる。ただし条件によっては同じbatch
  が2層で二重保持され、**100件→300件の重複配送**も再現した。
- flush完了・永続化ONといった「成功に見えるAPI応答」の裏で、**500件が全損**
  **・1,000件は末尾232件だけ残る**といったsilent lossが起きた。原因は
  Span件数の上限とbyte上限という別々の制約の衝突だった。
- 全21実験（E000〜E021）は生データ・SHA-256付きmanifest・再現スクリプトごと
  [GitHubで公開](https://github.com/siu01/otel-ios-reliability-lab)しており、
  どの主張が実機Simulatorで確認済みで、どれがモデル止まりかも機械的に監査
  している。
...
21実験を1行ずつまとめた。見出しは本文に登場する順番のままなので、気になる
実験があれば目次から本文へ飛んでもらいたい。

| ID | 何を検証したか | 結果 |
|---|---|---|
| **E000** | 正常系の対照実験 | 3条件とも100/100、測定器は壊れていない |
| **E001** | Collector 8秒後起動 | 永続化なし0件、Default 100件、Instant 200件(重複) |
| **E002** | 明示的flushを無効化 | 重複はむしろ300件に増加 |
| **E003** | HTTP requestを直接観測 | body 100→200→300へ増幅、trace/span IDも同一コピー |
| **E004** | retry所有者を1層に限定 | 3倍→1倍、欠損0を維持 |
| **E005** | プロセス終了→再起動 | Default/Instantとも100/100で回収 |
| **E006** | 書き込み境界の直接SIGKILL | ファイル完成前は0件、完成後は100件 |
| **E007** | forceFlush完了待ちでSIGKILL | 0/100→100/100、強いBarrierは不要だった |
| **E008** | 実`scenePhase.background`から検証 | flushなし0/100、flushあり45〜52msで100/100 |
| **E009** | 500/1,000 Spanへスケール | 500件は全損、1,000件は末尾232件だけ残存 |
| **E010** | `maxExportBatchSize`を100へ | 500件・1,000件とも全件回収 |
| **E011** | attributeを512B増やす | 同じbatch 100が100/100→0/100 |
| **E012** | chunkを50件へ分割 | 4条件すべて100/100に回復 |
| **E013** | batch 1でも起きる限界事例 | 256 KiBの単一Spanは分割不能で0/1 |
| **E014** | byte-aware policyの実装 | 既知の全損を全件回収、単一oversizeは明示rejection |
| **E015** | 分割探索をbinary searchへ | 同じ結果のままflushを85〜93%短縮 |
| **E016** | main queue再開までを計測 | flush完了後も224〜303ms追加で止まっていた |
| **E017〜E019** | byte policyのモデル拡張 | encode量73.5%減、並び順でobject数10→15、processor batch境界は越えられない(**実機runtime未実施**) |
| **E020〜E021** | 証跡とruntime主張の監査 | 孤立改ざんは検出可、同時書き換えは不可。runtime確認済み/未実施を機械的に分類 |
...
8秒障害で永続化なしの回収率は**0%**だった。公式Defaultは同じ条件で**100%**を
一意に回収した。つまり、アプリが生きている間の一時的な送信先障害は、設定だけで
0%から100%へ変えられた。

さらに、完全な永続ファイルを確認後にプロセスを終了しても、DefaultとInstantの
両方が新しいプロセスから**100件を一意に回収**できた。少なくともこの制御条件では、
...
E011はその100件という暫定値をattribute増加だけで**再び0/100へ壊した**。E012では
...
**0件だったSpanを100件回収**できた。しかしInstant presetと今回のOTLP/HTTP
exporterの組み合わせでは、失敗ごとに同じbatchを2層が保持し、成功requestが
**100→200→300 Spanへ増幅**した。HTTP側をstatelessにしてretryの所有者を
...
forceFlush完了後でも**500件は0、1,000件は末尾232件だけ**になった。APIの成功と
