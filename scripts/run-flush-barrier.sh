...
if [[ $# -ne 4 ]]; then
  echo "usage: scripts/run-flush-barrier.sh <evidence-run-id> <span-run-uuid> <persistence-mode> <flush-mode>" >&2
  exit 64
fi
...
case "$persistence_mode" in
  ...
  *)
    echo "flush-barrier run requires officialDefault or officialInstant" >&2
    exit 64
    ;;
esac
case "$flush_mode" in
  explicit|durabilityBarrier) ;;
  *)
    echo "flush mode must be explicit or durabilityBarrier" >&2
    exit 64
    ;;
esac
...
boundary_log="$run_dir/flush-boundary-state.tsv"
...
if curl --silent --fail http://127.0.0.1:13133/ >/dev/null 2>&1; then
  echo "collector is already running; E007 requires it to be unavailable initially" >&2
  exit 1
fi
...
printf 'trigger\tflush_completed_observed\n' >> "$boundary_log"
...
flush_completed_observed=false
for _ in {1..4000}; do
  if [[ -s "$lifecycle_path" ]] && rg -q '"phase":"flushCompleted"' "$lifecycle_path"; then
    flush_completed_observed=true
    break
  fi
  sleep 0.005
done
if [[ "$flush_completed_observed" != true ]]; then
  record_timing flush_completed_timeout
  echo "flush-completed lifecycle event did not appear within 20 seconds" >&2
  exit 1
fi
record_timing flush_completed_observed

if [[ ! -s "$app_run_dir/generated.jsonl" || ! -s "$app_run_dir/run.json" ]]; then
  echo "generation evidence is incomplete at flush completion" >&2
  exit 1
fi
generated_record_count="$(wc -l < "$app_run_dir/generated.jsonl" | tr -d ' ')"
if [[ "$generated_record_count" != "100" ]]; then
  echo "generated ledger does not contain exactly 100 records" >&2
  exit 1
fi
...
completed_flush_mode="$(jq -sr '[.[] | select(.phase == "flushCompleted")][0].flushMode' "$lifecycle_path")"
...
if [[ "$flush_started_count" != "1" || "$flush_completed_count" != "1" ]]; then
  echo "lifecycle evidence must contain exactly one flush start and completion" >&2
  exit 1
fi
if [[ "$completed_flush_mode" != "$flush_mode" ]]; then
  echo "lifecycle flush mode differs from the registered condition" >&2
  exit 1
fi
